...
class LoginTurnstile(session: KeycloakSession?) : UsernamePasswordForm(session) {
    ...
    override fun authenticate(context: AuthenticationFlowContext) {
        ...
        val config = context.authenticatorConfig?.config?.let { Turnstile.readConfig(it, DEFAULT_ACTION) }
        if (config == null) {
    ...
    override fun action(context: AuthenticationFlowContext) {
        ...
        if (webauthnAuth != null && webauthnAuth.isPasskeysEnabled() &&
            ...
            // webauthn form submission, try to action using the webauthn authenticator
